...
export const RightHeaderButtons = () => {
                ...
                <View style={{flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'center'}}>
                    <View style={[styles.filterTasks, {backgroundColor: TintCol(), borderColor: BackgroundCol()}]}>
                        ...
                        <TouchableOpacity style={styles.closeModalBtn} onPress={() => {setFilterModalVisible(!filterModalVisible)}}>
                            <MaterialCommunityIcons name="close-circle-outline" size={32} color={BackgroundCol()} />
                        </TouchableOpacity>
                        ...
                        <ScrollView style={styles.checkboxContainer}>
                            {filterCheckbox.map((mainFilter) => {
                                ...
                                return (
                                    <View style={{ borderWidth: 0, marginBottom: 15, padding: 0, backgroundColor: `${theme}10` }} key={mainFilter.id}>
                                        {/* Filter checkbox main */}
                                        <View style={[styles.checkboxMain, {backgroundColor: `${theme}25`}]}>
                                            <Checkbox
                                            ...
                                            return (
                                                <View style={[styles.checkboxSub, {borderBottomColor: `${theme}`}]} key={subFilter.id}>
                                                    <Checkbox
                                                        ...
                                                        color={`${theme}`}
                                                    />
                                                    <Text style={[styles.checkBoxHeader2, {color: `${theme}`}]}>{subFilter.header} {subFilter.label}</Text>
                                                </View>
                                            )
                                        })}
                                    </View>
    ...
    return (
        <View style={[styles.viewIcon,{backgroundColor:`${theme}20`,padding:4}]}>
            <TouchableOpacity onPress={() => {deleteTaskToggle()}} style={{paddingRight: 5}}>
                {!taskDeleteState ? (
                    <MaterialCommunityIcons name="checkbox-multiple-marked-outline" size={35} color={BackgroundCol()} />
                ) : (
            ...
            {viewMode == "list" ? 
                (
                    <TouchableOpacity onPress={() => {setViewMode("grid")}}>
                        <MaterialCommunityIcons name="view-grid-outline" size={35} color={BackgroundCol()} />
                    </TouchableOpacity>
                ):(
            ...
            <TouchableOpacity onPress={() => {setFilterModalVisible(!filterModalVisible)}}>
                <MaterialCommunityIcons name="filter-variant" size={35} color={BackgroundCol()} />
            </TouchableOpacity>
