...
const MenuBtn = ({onPress}) => {
    return (
        <TouchableOpacity style={{marginLeft: 10,backgroundColor:`${BackgroundCol()}20`,padding:4,marginBottom:4}} onPress={onPress}>
            <MaterialCommunityIcons name="menu-open" size={35} color={BackgroundCol()} />
        </TouchableOpacity>
    );
}
...
const TaskDetailBackBtn = ({onPress}) => {
    return (
        <TouchableOpacity style={{flexDirection: 'row', alignItems: 'center'}} onPress={onPress}>
            ...
            <Text style={{fontSize: 18, color: BackgroundCol(),width:"100%"}}>Back</Text>
        </TouchableOpacity>
    );
}
