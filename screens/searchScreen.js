            ...
            <TouchableOpacity onPress={() => { goTaskDetailScreen(item) }}
                style={[styles.taskTitleContainer_List,
                {
                    ...
                    borderWidth:
                       0,
                    backgroundColor:
                        item.priority == "High" ? "#DC143C40" :
                            item.priority == "Medium" ? "#FFD70040" :
                                item.priority == "Low" ? "#228B2240" : `${theme}40`,
                }
                ]}>
                {/* TASK TITLE */}
                <Text style={[styles.taskTitle_List,
                {
                    ...
                    textDecorationLine: item.complete ? 'line-through' : 'none',fontFamily:"Poppins-Regular"
                }]}>
                    {item.title}
                </Text>
                {/* TASK ICON */}
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: TintCol() }}>
            <View style={[styles.container, { marginBottom: tabBarHeight + 25 }]}>
                ...
                />
                {/* SEARCH RESULT LABEL */}
                <View style={{ borderBottomWidth: 0, borderColor: `${theme}`, marginBottom: "5%", backgroundColor: `${theme}40`, paddingHorizontal: 8, paddingVertical: 12 }}>
                    <Text style={{ fontSize: 20, color: `${theme}`,fontFamily:"Poppins-Bold" }}>
                        Search results
                    </Text>
                </View>
                {/* SHOW SEARCH RESULTS */}
                ...
                {filteredTaskList.length > 0 ?
                    ...
                    (
                        <Text style={{ fontSize: 16, color: darkmode ? "#ffffff" : "#000000",fontFamily:"Poppins-Regular" }}>
                            No search results
                        </Text>
                    )
                }
